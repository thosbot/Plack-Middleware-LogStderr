...
This middleware intercepts all output to C<stderr> and redirects it to a defined logger.
...
This middleware uses two techniques to catch messages sent to C<stderr> and direct
them to a logger.

One ties the C<STDERR> filehandle and directs all print messages to a logger.
This method only works if the code printing to C<STDERR> is aware of the perl tied
filehandle.

The second technique uses L<Capture::Tiny> to capture everything else written to C<stderr>.
For example any programs run using C<system>.
This method groups all C<stderr> output into one message.
The drawback here is log messages may not be interleaved temporally with messages
generated from the tied method or other calls to the logger.
...
=head2 C<tie_callback>


=head2 C<capture_callback>


Callbacks that take a string and return a string.

C<callback> is applied to all messages.

C<capture_callback> is applied to all messages logged via the capture method.

C<tie_callback> is applied to messages logged via the tied C<STDERR> filehandle.
...
The drawback is all C<stderr> output created during a request is grouped together as one message
and logged together after the request has finished processesing.
